...
public class BulkAndRestClient extends Client {

	public BulkAndRestClient(String site, String user, String password, String url) {
		super(site, user, password, url);
	}

	public static AccountInfo GetAccountInfo(String site, String user, String password) {
		Client client = new Client(site, user, password, "https://login.eloqua.com");
		Response response = client.get("/id");
		AccountInfo info = null;
		// System.out.printf("BULK AND REST CLIENT Client object", client);

		if (!((response.body).equals("\"Not authenticated.\""))) {
			info = client.Gson().fromJson(response.body, AccountInfo.class);

			System.out.println("BULK AND REST CLIENT Client INFOOOOOO" + info);
		}

		System.out.println("REssponse.body " + response.body);
		System.out.println("REssponse.Exception " + response.exception);
